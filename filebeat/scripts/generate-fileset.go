...
func main() {
	...
	if err != nil {
		log.Fatalf("Cannot generate module: %v\n", err)
	}

	err = generateFileset(*module, *fileset, *modulePath, *beatsPath)
	if err != nil {
		log.Fatalf("Cannot generate fileset: %v\n", err)
	}

	log.Printf("New module was generated. After setting up Grok pattern in pipeline.json, please run filebeat generate fields-yml\n")
}
